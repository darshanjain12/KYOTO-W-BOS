...
class SiteModel:
  ...
  class AssetColumns:
    ...
    POINTSET_POINTS = "udmi.pointset.points"
     ...
     # Used to create the "localnet" section.
    LOCALNET_FAMILIES = "udmi.localnet.families"
  ...
  def __init__(self, site_model_path):
    ...
    self.network_types = self._get_sheet_dict(excel_file, "network")
...
class UDMISiteModelGenerator:
  ...
  def _get_pointset_section(self, device):
    ...
    for point in self._site_model.payload_types:      
      ...
      if device[self._asset_columns.POINTSET_POINTS] == payload_points_type:
        points.update({
          payload_point_name: {
            "ref": payload_pointset_ref,"units": payload_pointset_units
          }
        })
  ...
  def _get_localnet_section(self, device):
    networks = {}

    for network in self._site_model.network_types:      
      payload_network_type = network[self._payload_type_columns.NETWORK_TYPE]
      payload_localnet_families = network[self._payload_type_columns.LOCALNET_FAMILIES]
      payload_localnet_id = network[self._payload_type_columns.LOCALNET_ID]

      if device[self._asset_columns.POINTSET_POINTS] == payload_network_type:
        networks.update({
          payload_localnet_families: {
            "addr": payload_localnet_id
          }
        })
  ...
  def generate(self):
    ...
    for device in self._site_model.assets:
      ...
      if device[self._asset_columns.DEVICE_OR_VIRTUAL] == "Device":
        ...
        metadata.update(self._get_localnet_section(device))
